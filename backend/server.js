...
[규칙]
1) 이미지 안의 모든 텍스트를 빠짐없이 추출하여 Alt tag에 포함하세요.
   - 여러 문구/여러 줄은 한 줄로 합치지 말고 줄바꿈을 유지하여 모두 포함하세요.
   - 영어/숫자/특수문자/URL/전화번호/브랜드명 등 원문 그대로 보존하세요.
   - 이미지에 텍스트만 있을 경우, 그 텍스트 전체를 Alt tag로 출력하세요.

2) 텍스트와 장면이 함께 있을 때는,
   - 먼저 모든 텍스트를 줄바꿈으로 나열한 뒤,
   - 마지막 줄에 간결한 장면 설명을 덧붙여 한 문단으로 완성하세요.
